...
function recipe_has_smelting_category(recipe)
  return x_util.contains(recipe.categories or {}, "smelting")
    or x_util.contains(recipe.categories or {}, "ll-fluid-smelting")
    or x_util.contains(recipe.categories or {}, "ll-electric-smelting")
end

-- Add heat to arc smelting recipes and make alternate recipes with heat for other smelting recipes
for name, recipe in pairs(data.raw.recipe) do
  if x_util.contains(recipe.categories or {}, "ll-arc-smelting") and name:sub(-5, -1) ~= "-heat" then
  ...
  elseif recipe_has_smelting_category(recipe) and recipe.name:sub(-5, -1) ~= "-heat" then
    ...
    heat_recipe.categories = {"ll-arc-smelting"}
