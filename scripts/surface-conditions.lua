...
local function on_tick(event)
  for _, robot in pairs(storage.robots_to_kill) do
    ...
    surface.spill_inventory{position = position, inventory = cargo_inventory, allow_belts = false, drop_full_stack = true}
    if repair_inventory then
      surface.spill_inventory{position = position, inventory = repair_inventory, allow_belts = false, drop_full_stack = true}
    end

    local items_to_place_this = robot.prototype.items_to_place_this
    if items_to_place_this then
      surface.spill_item_stack{position = position, stack = items_to_place_this[1], allow_belts = false, drop_full_stack = true}
    end
    robot.force.print({"ll-console-info.robot-cannot-be-used-on", robot.localised_name, {"?", {"space-location-name." .. surface.name}, surface.name}, "[gps=" .. position.x .. "," .. position.y .. "," .. surface.name .. "]"})
    robot.destroy()
  end
